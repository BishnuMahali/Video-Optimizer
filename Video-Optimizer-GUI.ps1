...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        ...
        Title="Ultimate Video Optimizer Pro v3.3.1" Height="920" Width="1200" Background="$($Theme.WindowBg)" WindowStartupLocation="CenterScreen">
    <Window.Resources>
    ...
    <Grid Margin="30">
        ...
        <Grid Grid.Row="1"><Grid.ColumnDefinitions><ColumnDefinition Width="450"/><ColumnDefinition Width="*"/></Grid.ColumnDefinitions>
            <ScrollViewer Grid.Column="0" VerticalScrollBarVisibility="Auto" Margin="0,0,20,0"><StackPanel>
                ...
                <Border Style="{StaticResource CardStyle}"><StackPanel><TextBlock Text="4. SESSION OPTIONS" FontWeight="Bold" Foreground="$($Theme.TextSub)" Margin="0,0,0,8"/><Grid Margin="0,0,0,8"><Grid.ColumnDefinitions><ColumnDefinition Width="*"/><ColumnDefinition Width="Auto"/></Grid.ColumnDefinitions><CheckBox x:Name="chkCacheResume" Content="Enable Cache &amp; Resume" IsChecked="True" VerticalAlignment="Center"/><Button x:Name="btnDelCache" Grid.Column="1" Content="&amp;#xE74D;" FontFamily="Segoe MDL2 Assets" Width="26" Height="26" Background="Transparent" BorderThickness="0" ToolTip="Delete Cache"/></Grid><Grid Margin="0,0,0,10"><Grid.ColumnDefinitions><ColumnDefinition Width="*"/><ColumnDefinition Width="Auto"/><ColumnDefinition Width="Auto"/></Grid.ColumnDefinitions><CheckBox x:Name="chkLog" Content="Enable Session Log" IsChecked="True" VerticalAlignment="Center"/><Button x:Name="btnDelLog" Grid.Column="1" Content="&amp;#xE74D;" FontFamily="Segoe MDL2 Assets" Width="26" Height="26" Background="Transparent" BorderThickness="0" Margin="4,0,0,0" ToolTip="Delete Log"/><Button x:Name="btnViewLog" Grid.Column="2" Content="&amp;#xE8C8;" FontFamily="Segoe MDL2 Assets" Width="26" Height="26" Background="Transparent" BorderThickness="0" Margin="4,0,0,0" ToolTip="View Log"/></Grid><Button x:Name="btnClearAll" Content="Clear Cache &amp; Data" HorizontalAlignment="Left" Background="Transparent" Foreground="#888888" BorderBrush="#555555" Padding="15,4"/></StackPanel></Border>
...
function Cleanup-Orphans {
    try {
        ...
        $tempPath = Join-Path $path "__Video-Optimizer__" | Join-Path -ChildPath "temp"
...
$btnDelCache.Add_Click({ $p = Join-Path $txtPath.Text "__Video-Optimizer__\Cache.json"; if (Test-Path $p) { Remove-Item -Force $p; Add-Log "[INFO] Cache deleted successfully." } else { Add-Log "[INFO] Cache file not found." } })
$btnDelLog.Add_Click({ $p = Join-Path $txtPath.Text "__Video-Optimizer__\Optimization_Log.txt"; if (Test-Path $p) { Remove-Item -Force $p; Add-Log "[INFO] Log deleted successfully." } else { Add-Log "[INFO] Log file not found." } })
$btnViewLog.Add_Click({ $p = Join-Path $txtPath.Text "__Video-Optimizer__\Optimization_Log.txt"; if (Test-Path $p) { Start-Process "notepad.exe" $p } else { Add-Log "[INFO] Log file not found." } })
$btnClearAll.Add_Click({ $p = Join-Path $txtPath.Text "__Video-Optimizer__"; if (Test-Path $p) { Remove-Item -Force -Recurse $p; Add-Log "[INFO] Completely cleared all cache and data." } else { Add-Log "[INFO] Data folder not found." } })
...
$btnStart.Add_Click({
    ...
    $config=@{
        ...
        CacheEnabled=$chkCacheResume.IsChecked
        CacheFile=$cacheFile
        Cache=$cache
        SettingsKey=$settingsKey
        ResumeEnabled=$chkCacheResume.IsChecked
